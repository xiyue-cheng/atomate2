...
class ArtatopWorkflowMaker(Maker):
    ...
    def make(self, structure: Structure, prev_dir: str | Path, additional_metadata: Optional[dict] = None) -> Flow:
        ...
        optics_job = band_structure_maker.make(structure=static_job.output.structure, prev_dir=static_job.output.dir_name)
        optics_job.name = "optics"

        # --- HSE ---
        
        kspacing_hse = getattr(optics_generator, "k_hse", 0.30)
        hse_generator = HSEStaticSetGenerator(
            user_incar_settings={
                "GGA": None, "NPAR": 8, "LAECHG": None, "LASPH": None,
                "LVTOT": None, "MAGMOM": None, "SIGMA": None, "ALGO": "Damped",
                "ENAUG": None, "LMIXTAU": None, "KSPACING": kspacing_hse,
                "KGAMMA": True, "LMAXMIX": True
            },
            user_kpoints_settings= None)
            
        hse_maker = HSEStaticMaker(input_set_generator=hse_generator)
        hse06_job = hse_maker.make(structure=static_job.output.structure, prev_dir=static_job.output.dir_name)
