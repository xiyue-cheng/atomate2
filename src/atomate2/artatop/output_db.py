from atomate2.artatop.schemas import ArtatopOutputModel
from pathlib import Path
from pymatgen.io.vasp import Vasprun

def parse_fixed_lines(input_file, cif_file_path):
    ...
    while idx < len(lines):
        ...
        if "linear optic epsilon at 0.000 eV" in line:
            ...
            d_tensor_data["0.000"] = [lines[idx+1].split(), lines[idx+2].split(), lines[idx+3].split()]
            idx += 4

        elif "d at omega = 0.650 eV" in line:
            d_tensor_data["0.650"] = [lines[idx+1].split(), lines[idx+2].split(), lines[idx+3].split()]
            idx += 4

        elif "d at omega = 1.167 eV" in line:
            d_tensor_data["1.167"] = [lines[idx+1].split(), lines[idx+2].split(), lines[idx+3].split()]
            idx += 4
    ...
    with open(output_file, "w") as f:
        ...
        for e in ["0.000", "0.650", "1.167"]:
            ...
            energy_label = e.replace("0.000", "0eV").replace("0.650", "0.65eV").replace("1.167", "1.167eV")
            f.write(f"{energy_label}   {n[0]} {n[1]} {n[2]} {biref}\n")

        f.write(f"deff at 0eV 0.65eV 1.167eV {deff_data.get('0.000', '0.0')} {deff_data.get('0.650', '0.0')} {deff_data.get('1.167', '0.0')}\n")

        # Correct d at omega energy labels with space
        d_energy_mapping = {
            "0.000": "0 eV",
            "0.650": "0.65 eV",
            "1.167": "1.167 eV"
...
def write_artatop_summary(output_model, cif_filename: str, filename_dir: str = "./"):
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"relax alpha (degree)           {output_model.relax_alpha or 0.0:.3f}\n")
        f.write(f"relax beta (degree)            {output_model.relax_beta or 0.0:.3f}\n")
        f.write(f"relax gama (degree)            {output_model.relax_gamma or 0.0:.3f}\n")
        f.write(f"relax Volume (A^3)             {output_model.relax_volume or 0.0:.3f}\n")

        f.write(f"V/Eg-EXP/Natom (A^3/eV)        {output_model.v_over_eg_exp_per_atom or 0.0:.3f}\n")
        f.write(f"V/Eg-HSE/Natom (A^3/eV)        {output_model.v_over_eg_hse_per_atom or 0.0:.3f}\n")

        f.write(f"Eg-PBE (eV)                    {output_model.bandgap_pbe or 0.0:.3f}\n")
        f.write(f"Eg-EXP (eV)                    {output_model.bandgap_exp or 0.0:.3f}\n")
        aexx_val = float(output_model.aexx_hse) if isinstance(output_model.aexx_hse, float) else 0.25
        f.write(f"Eg-HSE (eV) AEXX={aexx_val:.4f}        {output_model.bandgap_hse or 0.0:.3f}\n")
        f.write(f"AEXX use for HSE               {aexx_val:.4f}\n")
        f.write(f"Scissor-EXP (eV)               {output_model.scissor_exp or 0.0:.3f}\n")
        f.write(f"Scissor-HSE (eV)               {output_model.scissor_hse or 0.0:.3f}\n")
        ...
        for energy in [0.0, 0.65, 1.167]:
            lin_list = [l for l in (output_model.linear_response_uv or []) if abs(l.energy - energy) < 1e-3]
            if not lin_list:
                lin_list = [l for l in (output_model.linear_response_ir or []) if abs(l.energy - energy) < 1e-3]

            if lin_list:
                if abs(energy) < 1e-3:
                    f.write(f"e_xx at 0 eV                   {lin_list[0].real_part:.3f}\n")
                    f.write(f"e_yy at 0 eV                   {lin_list[1].real_part:.3f}\n")
                    f.write(f"e_zz at 0 eV                   {lin_list[2].real_part:.3f}\n")
                else:
                    f.write(f"n_xx at {energy:.3f} eV               {lin_list[0].refractive_index:.3f}\n")
                    f.write(f"n_yy at {energy:.3f} eV               {lin_list[1].refractive_index:.3f}\n")
                    f.write(f"n_zz at {energy:.3f} eV               {lin_list[2].refractive_index:.3f}\n")

            biref_list = (output_model.birefringence_uv or []) + (output_model.birefringence_ir or [])
            biref_val = next((b for b in biref_list if abs(b.energy - energy) < 1e-3), None)
            if biref_val:
                f.write(f"delta_n at {energy:.3f} eV             {biref_val.delta_n:.3f}\n")
        ...
        for energy in [0.0, 0.65, 1.167]:
            all_deff = (output_model.deff_values_uv or []) + (output_model.deff_values_ir or [])
            deff_val = next((d for d in all_deff if abs(d.energy - energy) < 1e-3), None)
            if deff_val:
        ...
        for d in (output_model.d_tensor_uv or []):
            if abs(d.energy) < 1e-3:
                component_keys = [
                    "d_xxx", "d_xyy", "d_xzz", "d_xyz", "d_xxz", "d_xxy",
                    "d_yxx", "d_yyy", "d_yzz", "d_yyz", "d_yxz", "d_yxy",
                    "d_zxx", "d_zyy", "d_zzz", "d_zyz", "d_zxz", "d_zxy"
                ]
                voigt_labels = [
        ...
        #f.write(f"Total energy from static (eV)  {output_model.total_energy_static:.3f if output_model.total_energy_static else 0.0}\n")
        total_energy = output_model.total_energy_static or 0.0
        f.write(f"Total energy from static (eV)  {total_energy:.3f}\n")
        f.write(f"NBANDS for static              {output_model.nbands_static if output_model.nbands_static else 0}\n")
        f.write(f"NBANDS for optic               {output_model.nbands_optics if output_model.nbands_optics else 0}\n")
    


def write_artatop_summary_result_line(output_model, cif_filename: str, filename_dir: str = "./"):

    """
    Write a summary line for an ARTATOP result using optical and structural information.

    Parameters:
    - output_model: ArtatopOutputModel containing parsed output data.
    - cif_filename: name of the original CIF file (used to extract material ID).
    - filename_dir: directory to write the RESULT-*.dat file into.
    """
    ...
    # Enthalpy = total energy per atom
    total_energy = output_model.total_energy_static or 0.0
    ...
    def get_deff(e):
        val = next((d.deff for d in (deff_uv_list + deff_ir_list) if abs(d.energy - e) < 1e-3), 0.0)
        return val
...
def write_artatop_analysis_file(output_model, cif_filename: str, filename_dir: str = "./"):
    ...
    highest_value = output_model.art_highest_value if output_model.art_highest_value is not None else 0.0
